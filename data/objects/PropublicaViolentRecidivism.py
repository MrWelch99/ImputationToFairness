...
class PropublicaViolentRecidivism(Data):
    ...
    def convert_ordinal_2_numerical(self, dataframe):
        ...
        for feature in ordinal_features:
            X = dataframe[feature].values.reshape(-1, 1)

            nan_free_mask = ~pd.isnull(X)
            nan_free_X = X[nan_free_mask]
            nan_free_X = nan_free_X.reshape(-1, 1)

            if feature == "age-cat":
                nan_free_encoded = encoder.fit_transform(nan_free_X)
                np.putmask(X, nan_free_mask, nan_free_encoded)
                ...
                nan_free_encoded = encoder_2.fit_transform(nan_free_X)
                np.putmask(X, nan_free_mask, nan_free_encoded)
