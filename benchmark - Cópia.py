        ...
        for dataset in dataset_lst:
            for imputation_method in imputation_method_lst:
                ...
                for start_run in start_run_lst:
                    temp_tuple = ([dataset],algorithm,num_trials,start_run,imputation_method,missing_mechaninsm,"missing_pipeline_baseline")
                    run_args.append(temp_tuple)

    l = mp.Lock()

    pool = mp.Pool(int(mp.cpu_count()/2)-2,initializer=init, initargs=(l,))
